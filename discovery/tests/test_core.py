...
def test_granicus_filters_and_normalizes():
    city = {
        ...
        "source_url": "https://example.com/ViewPublisher.php?view_id=2",
    ...
    with patch(
        ...
        return_value=(city["source_url"], GRANICUS_HTML),
    ):
        result = granicus.discover(city)

    meeting = result["latest_completed"]

    assert meeting.external_id == "976"
    assert meeting.meeting_date == "2026-08-12"
    assert meeting.recording_status == "found"
    assert "AgendaViewer.php" in meeting.agenda_url
