...
def test_spoken_foreign_item_title_is_a_boundary():
    candidate = (
...
def test_target_item_boundary_is_not_foreign():
    candidate = (
...
def test_clean_window_without_item_number_stays_eligible():
    candidate = (
        "OCTA provided an update on the I-5 El Toro Road "
        "interchange project and the council discussed congestion."
    )
...
def test_incidental_item_reference_is_not_a_transition():
    candidate = (
...
def test_nonformal_repair_rejects_foreign_transition_candidate():
...
def test_nonformal_repair_accepts_clean_i5_candidate():
    topic = "I-5 El Toro Road Interchange Project Update"
